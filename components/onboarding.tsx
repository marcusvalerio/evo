...
export function Onboarding({ onComplete }: { onComplete: (d: OnboardingData) => void }) {
  ...
  function Opt({ id,label,sub,active,onClick }:{id:string;label:string;sub:string;active:boolean;onClick:()=>void}) {
    return (
      <button onClick={onClick} className="press" style={{
        ...
        borderRadius: 8,
        boxShadow: active
          ? "0 0 0 1px #E34B26, inset 0 1px 0 rgba(181,158,95,0.1)"
          : "0 0 0 1px rgba(73,116,127,0.35)",
        marginBottom:6,cursor:"pointer",
        display:"flex",alignItems:"center",justifyContent:"space-between",
      }}>
        <div>
          <div style={{fontFamily:"var(--f-title)",fontSize:"0.82rem",textTransform:"uppercase",
            letterSpacing:"0.06em",color:active?"var(--hushed)":"var(--text)"}}>{label}</div>
        ...
        {active&&<Check size={14} style={{color:"#E34B26",flexShrink:0}}/>}
      </button>
    )
  }
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:100,background:"var(--canvas)",display:"flex",flexDirection:"column"}}>
      {/* Progress bar */}
      <div style={{height:2,background:"rgba(73,116,127,0.35)",flexShrink:0}}>
        <motion.div animate={{width:`${((step+1)/TOTAL)*100}%`}} transition={{duration:.35}}
          style={{height:"100%",background:"#E34B26"}}/>
      </div>
      {/* Header */}
      <div className="pt-safe" style={{padding:"14px 16px 0",flexShrink:0,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontFamily:"var(--f-logo)",fontSize:"1.1rem",color:"#E34B26",letterSpacing:"0.14em"}}>EVO</span>
        <span style={{fontFamily:"var(--f-title)",fontSize:"0.52rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)"}}>{step+1}/{TOTAL}</span>
      </div>
      {/* Content */}
      <div style={{flex:1,overflow:"hidden",position:"relative"}}>
        <AnimatePresence mode="wait">
          <motion.div key={step}
            ...
            {step===0&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.22em",color:"var(--text-2)",marginBottom:12}}>Sistema EVO</div>
                <h1 style={{fontFamily:"var(--f-title)",fontSize:"1.9rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"#E34B26",lineHeight:1.15,marginBottom:20}}>
                  Performance.<br/>Precisão.<br/>Evolução.
                </h1>
                <div style={{borderLeft:"2px solid #E34B26",paddingLeft:14,marginBottom:20}}>
                ...
                <div style={{padding:"10px 14px",background:"var(--container)",borderRadius:8,
                  boxShadow:"0 0 0 1px rgba(73,116,127,0.35)"}}>
            ...
            {step===1&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)",marginBottom:12}}>Calibração do Sistema</div>
                <h2 style={{fontFamily:"var(--f-title)",fontSize:"1.4rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"var(--text)",marginBottom:20}}>Parâmetros Iniciais</h2>
                {[
                  ...
                  <div key={f.key} style={{marginBottom:14}}>
                    <div style={{fontFamily:"var(--f-title)",fontSize:"0.52rem",textTransform:"uppercase",letterSpacing:"0.18em",color:"var(--text-2)",marginBottom:6}}>{f.label}</div>
            ...
            {step===2&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)",marginBottom:12}}>Vetor de Objetivo</div>
                <h2 style={{fontFamily:"var(--f-title)",fontSize:"1.4rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"var(--text)",marginBottom:20}}>Defina o Alvo</h2>
                {OBJETIVOS.map(o=><Opt key={o.id} id={o.id} label={o.label} sub={o.sub} active={data.objetivo===o.id} onClick={()=>setData(d=>({...d,objetivo:o.id}))}/>)}
              </div>
            )}
            {step===3&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)",marginBottom:12}}>Nível de Atividade</div>
                <h2 style={{fontFamily:"var(--f-title)",fontSize:"1.4rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"var(--text)",marginBottom:20}}>Carga Atual</h2>
            ...
            {step===4&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)",marginBottom:12}}>Parâmetros Adicionais</div>
                <h2 style={{fontFamily:"var(--f-title)",fontSize:"1.4rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"var(--text)",marginBottom:8}}>Calibração Fina</h2>
                ...
                <div style={{marginBottom:18}}>
                  <div style={{fontFamily:"var(--f-title)",fontSize:"0.52rem",textTransform:"uppercase",letterSpacing:"0.18em",color:"var(--text-2)",marginBottom:6}}>Restrições / Alergias</div>
                ...
                <div>
                  <div style={{fontFamily:"var(--f-title)",fontSize:"0.52rem",textTransform:"uppercase",letterSpacing:"0.18em",color:"var(--text-2)",marginBottom:10}}>Suplementos</div>
            ...
            {step===5&&(
              <div>
                <div style={{fontFamily:"var(--f-title)",fontSize:"0.55rem",textTransform:"uppercase",letterSpacing:"0.2em",color:"var(--text-2)",marginBottom:12}}>Sistema Configurado</div>
                <h2 style={{fontFamily:"var(--f-title)",fontSize:"1.4rem",textTransform:"uppercase",letterSpacing:"0.05em",color:"#E34B26",marginBottom:20}}>Telemetria Ativa</h2>
                <div className="metal-surface" style={{marginBottom:14}}>
                  {[
                    ...
                    <div key={r.l} style={{display:"flex",justifyContent:"space-between",padding:"10px 14px",borderBottom:i<4?"1px solid rgba(66,71,105,0.3)":"none"}}>
                      <span style={{fontFamily:"var(--f-title)",fontSize:"0.52rem",textTransform:"uppercase",letterSpacing:"0.18em",color:"var(--text-2)"}}>{r.l}</span>
