...
const QUICK = [
  ...
  { label:"PRÉ-TREINO",         q:"O que comer antes do treino? Algo prático com comida real." },
...
export function IAModal({ onClose, weights, evals, checked, userName, userGoal }: Props) {
  ...
  const system = `Você é o EVO Coach — sistema de orientação física e alimentar de alta performance.

Filosofia: comida real, pirâmide alimentar brasileira, sem industrializados. Suplementação apenas se mencionada (creatina e whey são ok). Não substitui médico ou nutricionista — mencione uma vez se relevante.

Perfil: ${userName||"não informado"} | Objetivo: ${userGoal||"não informado"} | Peso: ${lastW?`${lastW.val}kg`:"não registrado"} | Streak: ${streak}d | Aderência hoje: ${pct}%

Tom: técnico, direto, sem enrolação. Máximo 3 parágrafos. Sem markdown, sem asteriscos.`
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({
          ...
          system, messages:newMsgs.map(m=>({role:m.role,content:m.content})),
        }),
      })
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background:"var(--canvas-bg)",display:"flex",flexDirection:"column",
      }}>
      {/* Header */}
      <div className="pt-safe" style={{
        ...
        <div>
          ...
          <div style={{
            fontFamily:"var(--f-head)",fontSize:"0.65rem",
            textTransform:"uppercase",letterSpacing:"0.16em",
            color:"var(--text-primary)",marginTop:2,
          }}>COACH // SISTEMA ATIVO</div>
        </div>
        ...
        {msgs.length===0&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}}>
            <div style={{
              ...
              <p style={{
                fontFamily:"var(--f-body)",fontSize:"0.78rem",fontWeight:300,
                color:"var(--text-secondary)",lineHeight:1.65,
              }}>
                {streak>0
                  ? `${streak} dias no streak. Sistema operacional. Qual é a diretriz?`
                  : "Sistema pronto. O que você precisa?"}
              </p>
            </div>
            <div style={{
          ...
          {msgs.map((m,i)=>(
            <motion.div key={i}
              ...
              <div style={{
                ...
                background:m.role==="user"?"var(--accent-primary)":"var(--card-bg)",
                border:m.role==="user"?"none":"1px solid var(--border-color)",
                fontFamily:"var(--f-body)",
                fontSize:"0.8rem",fontWeight:300,
                color:m.role==="user"?"var(--canvas-bg)":"var(--text-primary)",
                lineHeight:1.6,
              }}>{m.content}</div>
            </motion.div>
          ))}
      ...
      {/* Input */}
      <div style={{
        borderTop:"1px solid var(--border-color)",padding:"12px 16px",
        flexShrink:0,display:"flex",gap:8,background:"var(--card-bg)",
      }} className="pb-safe">
        ...
        <button onClick={()=>send(input)}
          ...
          style={{
            ...
            border:"none",cursor:"pointer",opacity:input.trim()&&!loading?1:0.3,
            display:"flex",alignItems:"center",justifyContent:"center",
          }}>
          <ArrowRight size={15} style={{color:"var(--canvas-bg)"}}/>
        </button>
      </div>
    </motion.div>
  )
